...
- RPi는 수신한 `TOTAL`을 기준으로 1명, 2명, 이상 상태를 판단한다.
- 2인 상태가 4초 이상 유지되면 `BUZZ_ON`을 보낸다.
- `BUZZ_ON` 이후에도 10초 동안 2인 상태가 유지되면 `LOCK`을 보낸다.
- 무게가 정상 범위로 돌아오면 `BUZZ_OFF`를 보낸다.
- `LOCK`을 보내면 STM32는 릴레이와 부저를 끄고 무게 스트림도 종료한다.
- 잠금 이후 다시 무게를 보고 싶으면 `CHECK_WEIGHT`를 다시 보내서 스트림을 재시작해야 한다.
